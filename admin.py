...
def valid_nid(aad):
    if aad.isdigit() and len(aad)==10:
        return True
    return False


class login_page: #login page for admin
    ...
    def login(self, Event=None):# admin login validation
        username = user.get()
        password = passwd.get()
...
def about():
    ...
    page10 = About(about)
    page10.time()
    about.protocol("WM_DELETE_WINDOW", exitt)
    about.mainloop()
...
class Admin_Page:
    def __init__(self, top=None):
        ...
        self.message.configure(background="#00FFFF")
        ...
        self.button1.configure(command=self.Logout)# logout function is called
        ...
        self.button2.configure(command=inventory)# inventory function is called
        ...
        self.button3.configure(command=employee) #employee function is called
        ...
        self.button4.configure(command=invoices) #invoice function is called
        ...
        self.button5.configure(command=about) #about function is called
...
class Inventory:
    def __init__(self, top=None):
        ...
        self.button1.configure(command=self.search_product) #search_product is called
        ...
        self.button2.configure(command=self.Logout)#logout is called
        ...
        self.button3.configure(command=self.add_product)#add_product is called
        ...
        self.button4.configure(command=self.update_product)#update_product is called
        ...
        self.button5.configure(command=self.delete_product)#delete_product is called
        ...
        self.button6.configure(command=self.Exit)#exit is calle
        ...
        self.DisplayData()#display the raw inventory from db

    def DisplayData(self):#display the raw inventory from db
        cur.execute("SELECT * FROM raw_inventory")
        fetch = cur.fetchall()
        for data in fetch:
            self.tree.insert("", "end", values=(data))

    def search_product(self):#search product from the raw inventory from db
        val = []
    ...
    def delete_product(self):#delete product from the raw inventory from db
        val = []
        to_delete = []
    ...
    def update_product(self):#update product from the raw inventory from db
        if len(self.sel)==1:
            global p_update
            p_update = Toplevel()
    ...
    def add_product(self):#a product to the raw inventory in db
        global p_add
        global page4
        p_add = Toplevel()
    ...
    def Logout(self):# logout from the inventory page
...
class About:#about us
    def __init__(self, top=None):
        ...
        self.label1.place(relx=0, rely=0, width=1366, height=768)
        self.img = PhotoImage(file="./images/about_us.png") 
        self.label1.configure(image=self.img)
...
class add_product:#search product from the raw inventory from db
    def __init__(self, top=None):
        top.geometry("1366x768")
        top.resizable(0, 0)
    ...
    def clearr(self):#to clear the add page 
...
class Update_Product:#update product form db
    def __init__(self, top=None):
        top.geometry("1366x768")
        top.resizable(0, 0)
...
class Employee: #For employee management
    def __init__(self, top=None):
        top.geometry("1366x768")
        top.resizable(0, 0)
        ...
        self.tree.heading("NID No.", text="NID No.", anchor=W)
        ...
        if ename.strip():
            if valid_phone(econtact):
                if valid_nid(enid):
                    if edes:
                        if eadd:
                            if epass:
                                ...
                                insert = (
                                            "INSERT INTO employee(emp_id, name, contact_num, address, nid_num, password, designation) VALUES(?,?,?,?,?,?,?)"
                                        )
                                cur.execute(insert, [emp_id, ename, econtact, eadd, enid, epass, edes])
                                db.commit()
                    ...
                    messagebox.showerror("Oops!", "Invalid NID number.", parent=e_add)
            else:
                    ...
                    if edes:
                        if eadd:
                            if epass:
                                emp_id = vall[0]
                                update = (
                                            "UPDATE employee SET name = ?, contact_num = ?, address = ?, nid_num = ?, password = ?, designation = ? WHERE emp_id = ?"
                                        )
                                cur.execute(update, [ename, econtact, eadd, enid, epass, edes, emp_id])
                                db.commit()
