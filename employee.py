...
def random_bill_number(stringLength):#for generating the bill randomly
...
def login(Event=None):
    ...
    results = cur.fetchall() #return a list of tuple / empty
    if results:
        ...
        global page2
        biller = Toplevel()
        page2 = bill_window(biller) #bill_window class is called after login
        page2.time()
...
class login_page:
    def __init__(self, top=None):
        ...
        self.button1.configure(command=login) #login function is called to validate the login
...
class bill_window:
    def __init__(self, top=None):
        ...
        self.button1.configure(command=logout)#logout is called
        ...
        self.button2.configure(command=self.search_bill)#search_bill is called
        ...
        self.button3.configure(command=self.total_bill)#total_bill fun is called to show the biller info and to
        ...
        self.button4.configure(command=self.gen_bill) #gen_bill fuction is called
        ...
        self.button5.configure(command=self.clear_bill)#To clear the bill clear_bill is called
        ...
        self.button6.configure(command=exitt)#exit is called
        ...
        self.button7.configure(command=self.add_to_cart) #add_to_cart function is called
        ...
        self.button8.configure(command=self.clear_selection)#clear_selection is called
        ...
        self.button9.configure(command=self.remove_product)#remove_product is called

        text_font = ("Poppins", "8")
        self.combo1 = ttk.Combobox(biller) # Combobox is created for category
        ...
        self.combo2 = ttk.Combobox(biller) # Combobox is created for sub-category
        ...
        self.combo3 = ttk.Combobox(biller) # Combobox is created for item
        ...
        self.combo1.bind("<<ComboboxSelected>>", self.get_category) # get_category function is called
    ...
    def get_category(self, Event):
        ...
        self.combo2.bind("<<ComboboxSelected>>", self.get_subcat) #get_subcatagory function is called
        self.combo3.configure(state="disabled")
    ...
    def get_subcat(self, Event):
        ...
        self.combo3.bind("<<ComboboxSelected>>", self.show_qty) #show_quantity function is called
        self.entry4.configure(state="disabled")
    ...
    cart = Cart() # An object of Cart class is created
    def add_to_cart(self): #add_to_cart function is defined
        self.Scrolledtext1.configure(state="normal")
        strr = self.Scrolledtext1.get('1.0', END)
        if strr.find('Total')==-1: #Generate new bill/ No previous bill
            product_name = self.combo3.get()#choose a product first
            if(product_name!=""): #check if the product is selected
                product_qty = self.entry4.get() #choose the total No. of item
                find_mrp = "SELECT mrp, stock FROM raw_inventory WHERE product_name = ?" #Queru in database
                cur.execute(find_mrp, [product_name])#Queru in database
                results = cur.fetchall() #sotre the query result
                stock = results[0][1]
                mrp = results[0][0]
                if product_qty.isdigit()==True: #if product found in database
                    if (stock-int(product_qty))>=0: #checking the quantity is enough
                        sp = mrp*int(product_qty)#calculation price
        ...
        else: #clear the previous bill then create a new bill for new customer
    ...
    def remove_product(self): #remove a product form cart
    ...
    def wel_bill(self):# Function called for showing the biller details after generate
        self.name_message = Text(biller) #for showing the biller name
        ...
        self.num_message = Text(biller)#for showing number
        ...
        self.bill_message = Text(biller) #for showing bill number
        ...
        self.bill_date_message = Text(biller) #for showing date
    ...
    def total_bill(self):#function called to calculte the total bill
        if self.cart.isEmpty():
            ...
            if strr.find('Total')==-1:
                ...
                total = "\nTotal\t\t\t\t\t\t\t\t\t\t\tTaka. {}".format(self.cart.total())
    ...
    state = 1
    def gen_bill(self): #to generate the total bill
        if self.state == 1:
            strr = self.Scrolledtext1.get('1.0', END)
            self.wel_bill()
            if(cust_name.get()==""):
                ...
                if strr.find('Total')==-1:
                    ...
                    self.name_message.insert(END, cust_name.get())# predefined stringvar to store data
                    self.name_message.configure(state="disabled")
            
                    self.num_message.insert(END, cust_num.get())# predefined stringvar to store data
                    self.num_message.configure(state="disabled")
            
                    cust_new_bill.set(random_bill_number(8))# To generate bill

                    self.bill_message.insert(END, cust_new_bill.get())# predefined stringvar to store data
                    self.bill_message.configure(state="disabled")
                
                    bill_date.set(str(date.today()))# show dat
    ...
    def clear_bill(self):# clear the bill 
        self.wel_bill()
    ...
    def clear_selection(self): #clear the  selected bill 
    ...
    def search_bill(self):# search a existing bill
